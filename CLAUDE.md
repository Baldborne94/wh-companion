...
└── src/
    ...
    └── lib/
        ...
        ├── readerSettings.js ← reader defaults/persistence (device-derived two-page default)
...
⚠️ Reader settings defaults + persistence live in `lib/readerSettings.js` (`DEFAULTS`, `loadReaderSettings`, `saveReaderSettings`, `MIN_SPREAD_WIDTH`), not in `EpubReader.jsx` — so first-run rules can be tested without loading epub.js. The settings chips carry `aria-pressed` (a11y + the E2E handle for "which mode is active").
...
**Two projects**: `chromium` (Desktop Chrome viewport) runs everything except `mobile.spec.js`; `mobile-chromium` (Pixel 7 — touch pointer, phone viewport) runs *only* `mobile.spec.js`. The app is used mostly on a phone, where several rules differ and can break on their own — touch starts the reader immersive, two-page is off by default, and anything wider than the viewport shows up as a sideways-scrolling page — none of which a desktop-viewport run can see. The mobile project is deliberately scoped to one spec so CI stays fast; the rest of the behaviour is viewport-independent and already covered once.
