...
Reaching the last page auto-marks the book **read** (so it appears on the shelf, and reading stats + achievements update). The readers signal completion via an `onFinish` prop — `EpubReader` fires it once when epub.js `relocated` reports `loc.atEnd`; `PdfReader` fires it when `page >= total`. `App.markBookFinished(book)` then calls `updateStatus`/`updateAoSStatus(book.id, 'read')` (picking the map by the active `universe`), guarded so an already-read book isn't re-written. `setBookStatusLS` stamps `completedAt`; the change persists to localStorage + Supabase `reading_status`.
